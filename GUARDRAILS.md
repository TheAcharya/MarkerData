...
| **Extract** (Workflow Extension) | **“Drop to Open Marker Data”** | default | `WorkflowExtensionView` + `isExtractDropTargeted`; `subtitle: nil`; handoff writes Movies-cache FCPXML then opens the app. Idle hint: *“Drag & Drop Final Cut Pro Project to Open Marker Data”*. |
...
9. **Icon Composer dialogs:** SwiftUI `.alert` still needs `.appDialogIcon()` (`MarkerDataAppIcon` from compiled `Marker-Data.icon`). Do not flatten the layer PNG into `AppIconSingle` (skips glass / fill). Workflow Extension **header** loads the icon from the containing `Marker Data.app` — the appex’s `applicationIconImage` is its `AppIcon.appiconset` (or Final Cut Pro). Leave the extension’s bundle/plugin `AppIcon.appiconset` unchanged.
