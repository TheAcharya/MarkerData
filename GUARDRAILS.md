...
24. **Unused throwing unstructured `Task` (Xcode 27 / `#NoUseUnstructuredThrowingTask`).** `Task { try await … }` without storing the task or catching inside silently drops errors. Handle with `do/catch` (or drop a spurious `throws` / `try`) so the task is non-throwing; on `@MainActor` types, call directly. Do not silence with `_ = Task`.
